...
Guia detalhado de **onde está cada coisa, para que serve e como entra/sai** — o **resumo**
navegável do grafo (§39). Obrigatório por `references/padroes-codigo.md` (§4). **Mora em
`<projeto>/<projeto>_archive/index/MAPA.md`, nunca no root** (§28), como **espelho durável**; a versão
**operacional que o app desenha** é `.schematize/grafos/` (`GRAFO_GLOBAL.md` + `<servico>.md`).
Atualize no mesmo PR que mexe no código (o archive é versionado). Duas camadas:

- **Global** — mantido à mão (repos/serviços como nós com funções principais, contratos como arestas).
- **Microfunções** — gerado por `/<slug>-index` a partir dos doc-comments.

> **Exaustivo (não resumo):** uma entrada **por função** — `nº de entradas == nº de funções` do serviço. O MAPA é um **grafo**, não uma lista: inclui o **grafo global** (§2.5) e o **grafo de chamadas** (§5). Arestas SEMPRE em ASCII `A -> B (contrato)`, **nunca** `→` — é o que o parser do app lê (§39).
