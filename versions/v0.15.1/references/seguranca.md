...
- Guardar token de sessão em `localStorage`/`sessionStorage`. Sessão vai em **cookie `HttpOnly` + `Secure` + `SameSite`** (§38; hardening — **schematize-pentest**).
...
- CSP, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy` e `frame-ancestors` configurados (hardening — **schematize-pentest**).
...
- Confiar em `redirect`/`next` param sem allowlist (open redirect — **schematize-pentest**).
