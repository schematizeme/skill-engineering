...
**`.overdev/DECISOES.md`** (+ espelho no `<projeto>_archive/overdev/`) em formato **ADR-lite**:
...
produza o **PLANO** em **`.overdev/PLAN.md`** (+ archive):
...
- **Fonte de dados (o control-plane da Fase 0):** `.overdev/DECISOES.md`, `.overdev/PLAN.md`,
  `.overdev/CHECKLIST.md`, `./PERGUNTAS-OVERDEV.txt` e o **grafo do index**
  (`<projeto>_archive/index/`). O painel **lê** esses arquivos — não é fonte da verdade, é vista.
