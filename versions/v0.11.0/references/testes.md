...
**Guarda provada no vermelho (MUST — anti "verde mentiroso")**
- **Todo teste/guarda tem que ser visto FALHAR quando o invariante que ele protege quebra.** Um teste que passa verde **sem nunca ter reprovado o caso ruim** não é guarda — é decoração. Escreva-o **red-first** (ou quebre o código de propósito uma vez) e confirme que fica vermelho. Bug clássico: a asserção casa com o **vizinho** (o filtro/array errado, um regex que bate em outra coisa) e "passa" sem checar nada.
- **O runner tem que ENXERGAR o teste.** Confirme que os arquivos novos são **coletados** (os globs de `include` cobrem `*.test.tsx`/`*.test.ts`/`_test.go`/`tests/**`; nada de teste fora do glob). Suíte verde que rodou **0** dos testes novos é falso-verde silencioso — falhe o CI em "no tests found" no caminho novo e/ou asserte a **contagem esperada** de casos.

**Mutation testing (SHOULD)** no domínio em serviços críticos: Stryker (JS/TS), `go-mutesting`, `cargo-mutants` — a rede que pega a guarda que não guarda quando o red-first passou batido.
