...
20a. **Guarda que nunca foi vista falhar** (teste que passa verde sem nunca reprovar o caso ruim — a asserção casa com o vizinho/array/regex errado), **ou teste que o runner nem coleta** (fora do glob de `include` → não roda, não avisa).
    → **Red-first:** prove o vermelho antes do verde; confirme que o runner **enxerga** o arquivo (globs cobrem `.test.tsx`/`_test.go`/etc.) e falhe em "no tests found" (§22).
...
47. **Gate que cria estado sem saída (deadlock de bootstrap)** — exigir 2º fator para entrar **e** exigir sessão para enrolar o 2º fator; oferecer como "saída" um link para página que pede justamente o que falta; ou mandar "peça ao admin" enrolar o autenticador **pessoal** do usuário. Sintoma real: N contas presas sem TOTP porque a única porta pedia o que elas não tinham.
    → Toda exigência tem **rampa self-service alcançável a partir do estado zero** — o fallback always-on (Email OTP) dá a sessão de baixo AAL para **enrolar** o fator forte. Prove por estado: nenhum estado de conta sem transição de saída (`references/iam.md` §4; teste de reachability em schematize-pentest `iam-testing.md`).
