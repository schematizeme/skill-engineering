...
0.1 **Colha as decisões ACORDADAS no contexto atual** — varra a sessão inteira e extraia o que
   ...
   `.overdev/DECISOES.md` (+ espelho no `<projeto>_archive/overdev/`). Trava o que já foi
...
0.2 **Carregue o GRAFO do index** — rode/leia `/eng-index` (§39): `MAPA.md` + grafos em
   `<projeto>_archive/index/` (adjacência `A -> B`). Ancore o plano no grafo real (cada item
   aponta nó/`arquivo:linha` e arestas afetadas). Sem index? gerá-lo é o **1º item** do checklist.

0.3 **Planejamento PESADO** → `.overdev/PLAN.md` (+ archive): escopo (entra/NÃO entra),
...
0.4 **Derive o CHECKLIST do plano** → `.overdev/CHECKLIST.md` (+ espelho `OBJETIVO.md`).
...
1. **Ative o run** com o CLI (motor Rust — ver `references/overdev.md` §7):
   ...
   - **Grave os artefatos da Fase 0** em `.overdev/`: `DECISOES.md` (0.1), `PLAN.md` (0.3) e o
     `CHECKLIST.md` **derivado do plano** (0.4, sobrescreve o template); espelhe `PLAN.md` +
     `OBJETIVO.md` em `<projeto>_archive/overdev/` (registro durável §28).
