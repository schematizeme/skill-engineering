...
#        roda /eng-overdev (que cria .schematize/overdev/state mode=active).
#
...
#   $SD/state        KEY=VALUE: mode=active, max_iters=N, archive=<path>
#   $SD/CHECKLIST.md  checklist com "- [ ]" (aberto) / "- [x]" (feito)
...
STATE="$SD/state"; CL="$SD/CHECKLIST.md"
...
open=0
if [[ -f "$CL" ]]; then open="$(grep -cE '^[[:space:]]*- \[ \]' "$CL" 2>/dev/null)"; [[ "$open" =~ ^[0-9]+$ ]] || open=0; fi
gate_ok=1; gate_msg=""
...
if (( open == 0 )) && (( gate_ok == 1 )); then
  allow "checklist 100% e gate verde — o agente pode declarar concluído (crie $SD/DONE)."
fi
