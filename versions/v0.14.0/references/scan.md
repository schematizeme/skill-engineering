...
- **Testes:** caminho crítico sem teste, cobertura abaixo do piso, **guarda que não guarda** (teste verde que nunca reprovou o caso ruim — cruza `testes.md` red-first), teste que o runner não coleta (glob de include), smoke sem self-check.
...
4. **Triagem e saída machine-readable:** cada achado = `{classe, arquivo:linha, severidade (bloqueia/alta/média/baixa/info), descrição, piso violado, conserto sugerido, veredito(confirmado/suspeito)}`. Grava em `<projeto>_archive/scan/<data>.md` (+ JSON pro CI, se houver).
...
- **Vira reteste:** o achado consertado ganha um teste que impede a regressão (cruza `testes.md`).
- **Fecha no audit:** o `/eng-audit` confere depois que os itens do scan foram **de fato** sanados (não só marcados).
- **Archive obrigatório** (§28): o relatório de scan mora em `<projeto>_archive/scan/`, nunca no root.
