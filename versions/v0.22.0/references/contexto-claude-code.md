...
> Parte da skill **schematize-engineering**. **É o DONO do handoff de contexto**: o §34.1 do `entrega.md` **da base** (`schematize-engineering`) é o *gatilho* (quando parar e arquivar); o *como* — formato dos dois MDs, hooks, `/compact`, o que entra em cada seção — é aqui, e só aqui. Implementa também a §28 (archive). Conteúdo específico de ferramenta (Claude Code / VS Code) — o princípio normativo está no `entrega.md` **da base** (`schematize-engineering` -> `references/entrega.md` §34.1) — o recorte local deixou de repeti-lo quando virou ponteiro.

## Por que

Em sessão longa o contexto enche e a compactação automática resume de forma lossy — decisões se perdem, a qualidade cai. A regra: **handoff proativo e arquivado antes do teto**, sempre gravado em `<projeto>/<projeto>_archive`.
...
1. **StatusLine monitor** (`scripts/hooks/context-monitor.mjs`) — detecta o cruzamento do `CTX_THRESHOLD`, acende um flag visível na barra e grava um backup bruto de segurança em `<projeto>/<projeto>_archive/context/`.
...
4. Exporte `PROJECT_ARCHIVE=<caminho>/<projeto>/<projeto>_archive/context` e, se quiser, `CTX_THRESHOLD`.
...
## Comando `/eng-cc` (context compact) — handoff manual sob demanda

No Claude Code, comandos e skills são unificados: ambos criam um `/nome`. Há dois formatos — `.claude/commands/<nome>.md` (legado, mais simples) e `.claude/skills/<nome>/SKILL.md` (recomendado; suporta `/nome` mais invocação autônoma, e aceita o campo `command:` no frontmatter). Uma pasta de skill = um comando, então um `/eng-cc` próprio é um arquivo de comando dedicado.

Bundlado em `assets/commands/eng-cc.md`. Instalação:

1. Copie `assets/commands/eng-cc.md` para `.claude/commands/eng-cc.md` na raiz do projeto.
2. Digite `/eng-cc` na sessão (ou `/eng-cc <foco>` pra direcionar o resumo, via `$ARGUMENTS`).

O `/eng-cc` gera dois MDs em `<projeto>/<projeto>_archive/context/`:
...
E então roda `/compact` com foco. Diferença pro automático: o StatusLine/PreCompact é o gatilho **automático** no limite; o `/eng-cc` é o handoff **manual** que você dispara quando quiser fechar um marco. Os dois gravam no mesmo `<projeto>/<projeto>_archive/context/`.

Alternativa em formato skill (auto-invocável além do `/eng-cc`): criar `.claude/skills/eng-cc/SKILL.md` com frontmatter `name`, `description` e `command: /eng-cc`, e o mesmo corpo do `cc.md`.
