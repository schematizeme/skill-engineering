...
## 3.1 Complexidade — os dois gates que o CI mede além do tamanho

Linha de código é proxy **ruim** para complexidade: um arquivo de 200 linhas com um `switch` de 30
ramos e 6 níveis de `if` é pior que um de 600 linhas lineares. Por isso, além do teto de tamanho
(§1), o CI mede a coisa direta:

- **Complexidade ciclomática > 15** em função de produção → **bloqueia**. É o número de caminhos
  independentes; acima de 15 ninguém consegue enumerar os casos de teste de cabeça, e o teste
  passa a cobrir o caminho feliz por omissão.
- **Aninhamento > 4 níveis** → **bloqueia**. Aninhamento profundo é quase sempre *guard clause* que
  não foi escrita: inverta a condição e retorne cedo.

**Exceções** (as mesmas de §1): teste, migration, código gerado, schema/fixture.

**Onde isso roda:** no linter da linguagem, no gate do CI — não na revisão humana. Exemplos:
`gocyclo`/`nestif` no `golangci-lint` (Go), `clippy::cognitive_complexity` (Rust),
`complexity`/`max-depth` no ESLint (TS), `Metrics/CyclomaticComplexity` no RuboCop (Ruby),
`Credo.Check.Refactor.CyclomaticComplexity` (Elixir), analisadores do `.editorconfig` (C#).

> Estes dois pisos existiam **apenas** em `arquitetura.md` §6, declarados como bloqueio de CI e
> ausentes daqui, do `check-diff.sh` e do config de lint — ou seja, um "bloqueio" que nada media
> (achado do inventário da vistoria de 2026-08-21). Promovidos para cá, que é o arquivo que o gate
> lê, e refletidos no `assets/lint/.golangci.yml`.

## 4. MAPA da aplicação (arquivo-guia obrigatório)

Todo projeto que segue esta skill mantém um **`MAPA.md`** em **`<projeto>/<projeto>_archive/index/MAPA.md`** (template em
...
- [ ] `MAPA.md` atualizado no mesmo PR, em **`<projeto>/<projeto>_archive/index/`** (nunca no root) — camada global à mão + microfunções gerada.
