...
metadata:
  version: 0.22.0
description: Padrões normativos de engenharia da casa — a BASE comum, agnóstica de linguagem, que se repete em toda a stack. Backend em ROL SANCIONADO por fit + ADR (Go, Rust, Elixir, C#, Zig, Ruby — cada uma com skill irmã; Node-backend e PHP em saída); o piso é o mesmo em toda linguagem: arquitetura/DDD, MAPA+grafo+endpoints, segurança, IAM, dados/eventos, cadeia de suprimentos, observabilidade, operação/DoD/archive, orquestração com subagents, anti-padrões — mais as disciplinas de fluxo (scan, planejamento, refactor, auditoria de histórico, overdev contínuo). Use SEMPRE que for projetar, gerar, revisar ou refatorar software, decidir arquitetura, escolher stack, modelar eventos/banco, configurar observabilidade, paralelizar, VARRER a base, PLANEJAR antes de construir, REFATORAR sem quebrar comportamento, AUDITAR checklists ou produzir ADR/runbook/archive. Pisos: segredo nunca no cliente; sem SQL concatenado; auth server-side; índice exaustivo; archive obrigatório; efeito externo nunca sai de não-prd.
---
...
**Versão:** skill `schematize-engineering` v0.22.0. Changelog em `CHANGELOG.md`.
...
| **EFEITOS EXTERNOS fora de prd (e-mail/SMS/push/webhook/pagamento): sink por default, domínio de teste em ROTA NULA (null MX + SPF `-all` + DMARC reject), guard deny-by-default no provider, cap por execução, 4 camadas** | `references/efeitos-externos.md` |
...
- **Efeito externo NUNCA sai de não-produção (`references/efeitos-externos.md`).** E-mail, SMS/voz, push, webhook de terceiro e cobrança **não acontecem de verdade** fora de `prd` — por construção, não por lembrança. **(a)** Todo endereço sintético (fixture, seed, persona, demo, carga) usa o **domínio de teste em ROTA NULA** — `test.<domain>` com **null MX (RFC 7505) + SPF `-all` + DMARC `p=reject`**, ou TLD reservado (`.test`/`.invalid`/`.example`); **VETADO** `@gmail.com`/domínio de terceiro/e-mail de pessoa real (inclusive o seu) e o domínio de produção. **(b)** O provider **default é o SINK** (Mailpit/log) fora de prd, e o **guard mora DENTRO do provider**: destinatário externo com `env != prd` → **erro**, fail-closed (config ausente = assume não-prd). **(c)** **Cap por execução** (`MAIL_MAX_PER_RUN`, default 50) + circuit breaker que **aborta** — o disparo em massa só existe porque nada contava. **(d)** Chave de não-prd é **sandbox**, egress SMTP bloqueado em dev/hml. Entregar de verdade fora de prd exige **ADR + allowlist ≤5 + cap + janela + subdomínio separado** — as cinco, ou não roda. Motivo: bounce/complaint em massa **queima IP e domínio**, derruba o e-mail transacional de prd (inclusive o **OTP de login**) e leva semanas de warm-up pra recuperar — com utilidade **zero**. Anti-padrão espelho na §37 (item 49).
...
- `scripts/check-external-effects.sh` — **gate do piso de efeito externo** (§37, *"Disparar efeito externo REAL a partir de não-produção"*): trava se achar endereço de caixa real (`@gmail.com` etc.) em test/seed/fixture/persona, credencial de provedor de envio em `.env` de não-prd que não seja sandbox, envio sem cap por execução, ou (com `TEST_MAIL_DOMAIN` + `dig`) domínio de teste **sem null MX**. Já plugado no `assets/ci/github-actions-ci.yml`.
