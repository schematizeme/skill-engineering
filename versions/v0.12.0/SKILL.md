...
metadata:
  version: 0.12.0
description: Padrões normativos de engenharia da casa — a BASE comum, agnóstica de linguagem, que se repete em toda a stack (arquitetura/DDD, MAPA+grafo+endpoints, segurança, dados/eventos, cadeia de suprimentos, observabilidade, operação/DoD/archive, orquestração/paralelização com subagents, anti-padrões, gestão de contexto). Use SEMPRE que for projetar, gerar, revisar ou refatorar qualquer software, decidir arquitetura, escolher stack, modelar eventos/banco, configurar observabilidade, planejar execução paralela/multiagente, ou produzir ADR/runbook/archive — mesmo sem citar "padrão". Contém pisos inegociáveis (segredo nunca no cliente, sem SQL concatenado, auth server-side, índice/MAPA exaustivo, archive obrigatório, tempo do usuário acima de tokens) que vetam atalhos inseguros e execução serial ineficiente. Detalhe por linguagem nas skills irmãs: schematize-go/rust (backend), schematize-web (frontend), schematize-node (legado). Pentest é a schematize-pentest.
---
...
**Versão:** skill `schematize-engineering` v0.12.0. Changelog em `CHANGELOG.md`.
