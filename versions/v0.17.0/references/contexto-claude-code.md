...
O `/eng-cc` gera dois MDs em `<projeto>_archive/context/`:
...
E então roda `/compact` com foco. Diferença pro automático: o StatusLine/PreCompact é o gatilho **automático** no limite; o `/eng-cc` é o handoff **manual** que você dispara quando quiser fechar um marco. Os dois gravam no mesmo `<projeto>_archive/context/`.
